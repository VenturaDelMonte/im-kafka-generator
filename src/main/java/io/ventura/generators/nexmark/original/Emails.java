...
public class Emails {
    ...
    public static final int NUM_EMAILS = EMAILS.length;

    private static final Charset US_ASCII = Charset.forName("US-ASCII");

    public static final byte[][] EMAILS_32 = new byte[NUM_EMAILS][];
    static {
    	for (int i = 0; i < NUM_EMAILS; i++) {
    		EMAILS_32[i] = EMAILS[i].substring(0, Math.min(32, EMAILS[i].length())).getBytes(US_ASCII);
		}
	}
}
